...
class ResCurrencyRate(models.Model):
    ...
    def _get_latest_rate_2(self):
        if not self.name:
            raise UserError(_("The date for the current rate is empty.\nPlease set it."))
        return self.currency_id.rate_ids.sudo().filtered(lambda x: (
            x.rate_2 and x.company_id == (self.company_id or self.env.company) and x.name < (self.name or fields.Date.today())
        )).sorted('name')[-1:]

    def _get_last_rates_for_companies_2(self, companies):
        return {
            company: company.currency_id.rate_ids.sudo().filtered(lambda x: (
                x.rate_2 and x.company_id == company or not x.company_id
            )).sorted('name')[-1:].rate_2 or 1
            for company in companies
        }
    ...
    # El resto de métodos como _cron_update y _create_the_latest_exchange_rate_to_date quedan sin cambios
